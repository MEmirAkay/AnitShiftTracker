...
export default class SignInVerify extends Component {
    ...
    calcTime(time) {
        ...
        const h = Math.floor(time / 3600), m = Math.floor(time % 3600 / 60), s = Math.floor(time % 3600 % 60);
        ...
        return (
            <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false} >
                <View style={styles.container} >
                    ...
                    <Text style={styles.text}>Yetkili ekibimizle iletişime geçerek adınıza gelen kodu alabilirsiniz.</Text>
                    ...
                    {
                        (this.state.cntrl && this.state.verifyCode.length === 8) ? (<this.Loading />) :
                            (<TouchableOpacity
                                style={styles.btnContainer}
                                disabled={(this.state.verifyCode.length === 8) ? false : true} onPress={() => {
